...
class WidgetPositioner:
    def __init__(self, root):
        ...
        self.save_button = tk.Button(
            root, text="Save Positions", command=self.save_positions
        )
    ...
    def create_widgets(self):
        ...
        tk.Button(self.size_frame, text="Apply", command=self.adjust_size).pack(
            side="left"
        )
        ...
        if self.dragged_widget:
            try:
                ...
                self.canvas.itemconfig(
                    self.dragged_widget, font=("Digital-7", new_size, "bold")
                )
