        ...
        try:
            for sensor_type in ["temperature", "wind_speed", "wind_direction"]:
                ...
                self.gui1_canvas.itemconfig(
                    self.gui1_widgets[sensor_type], text=formatted_value
                )
            ...
            if humidity is not None:
                ...
                self.gui1_canvas.itemconfig(
                    self.gui1_widgets["humidity"], text=formatted_value, fill=color
                )
                self.gui1_canvas.itemconfig(
            ...
            if wind_dir is not None:
                ...
                self.gui1_canvas.itemconfig(
                    self.gui1_widgets["wind_direction_cardinal"], text=cardinal
                )
        except Exception as e:
        ...
        try:
            ...
            if uv is not None:
                ...
                self.gui2_canvas.itemconfig(
                    self.gui2_widgets["uv"], text=f"{uv:.2f}", fill=uv_color
                )
                self.gui2_canvas.itemconfig(
                    self.gui2_widgets["uv_state_value"], text=uv_state, fill=uv_color
                )
            ...
            if pm2_5 is not None:
                ...
                self.gui2_canvas.itemconfig(
                    self.gui2_widgets["aqi"], text=f"{aqi:.0f}", fill=aqi_color
                )
                self.gui2_canvas.itemconfig(
                    self.gui2_widgets["aqi_state_value"], text=aqi_state, fill=aqi_color
                )
            ...
            for sensor_type in ["pressure", "rain"]:
                ...
                formatted_value = config["display_format"](value)
                self.gui2_canvas.itemconfig(
                    self.gui2_widgets[sensor_type], text=formatted_value
                )
