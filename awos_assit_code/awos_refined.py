...
class WeatherStationSystem:
            ...
            for filename in os.listdir(logs_dir):
                if filename.startswith("weather_station_") and filename.endswith(
                    ...
                    try:
                        ...
                        file_date_str = filename.replace(
                            "weather_station_", ""
                        ).replace(".log", "")
        ...
        try:
            ...
            for filename in os.listdir(self.csv_dir):
                if filename.startswith("weather_data_") and filename.endswith(".csv"):
                    try:
                        # Extract date from filename
                        file_date_str = filename.replace("weather_data_", "").replace(
                            ".csv", ""
                        )
                        file_date = datetime.strptime(file_date_str, "%Y-%m-%d").date()
                        ...
                        self.log(
                            f"Error processing CSV file {filename}: {e}",
                            level=logging.ERROR,
                        )
    ...
    def setup_gui(self):
        ...
        try:
            ...
            if not os.path.exists(bg_path):
                self.log(
                    f"Background image not found at: {bg_path}", level=logging.ERROR
                )
    ...
    def store_daily_rainfall(self, total):
        """Store the daily rainfall total into a CSV file."""
        try:
    ...
    def process_rainfall(self, current_rain):
        """Accumulate rainfall throughout the day and reset at midnight.
        ...
        if (
            ...
            if hasattr(self, "daily_rain_total"):
                ...
                self.log(
                    f"Rainfall reset at midnight. Stored: {self.daily_rain_total:.1f} mm"
                )
        ...
        if rain_increment >= 0:
            ...
            self.log(
                f"Rain sensor reset or error. Current: {current_rain}, Last: {self.last_rain_value}"
            )
    ...
    def sensor_reader_loop(self):
        ...
        while self.running:
            try:
                if not self.modbus_client.connect():
                    self.log(
                        "Modbus connection failed sensor reader loop",
                        level=logging.ERROR,
                    )
                    time.sleep(5)
                    continue
                ...
                for sensor_name, reader in sensors:
                    try:
                        ...
                        self.log(
                            f"Error reading {sensor_name}: {e}", level=logging.ERROR
                        )
    ...
    def log_sensor_data(self, sensor_name, data):
        ...
        if sensor_name == "environment":
            self.log(
                f"Env: {data['temperature']:.1f}°C, {data['humidity']:.1f}%, {data['pressure']:.1f}hPa"
            )
        elif sensor_name == "uv":
            ...
            self.log(
                f"Wind Direction: {data['wind_dir_degrees']}° ({data['wind_dir_cardinal']})"
            )
    ...
    def read_aqi_sensor(self):
        ...
        try:
            ...
            try:
                ...
                # Get the closest timestamp row
                closest_time_row = df.iloc[
                    (df["date"] - current_time).abs().argsort()[0]
                ]
        ...
        while self.running:
            try:
                ...
                csv_file = os.path.join(
                    self.csv_dir, f"weather_data_{current_date}.csv"
                )
    ...
    #         self.log(f"Current sensor data: {self.sensor_data}")
            ...
            for sensor_name, config in self.sensor_configs.items():
                try:
                    ...
                    self.bg_canvas.itemconfig(
                        getattr(self, config["widget"]), text="--"
                    )
    ...
    def update_state_displays(self):
        ...
        try:
            ...
            self.bg_canvas.itemconfig(
                self.humidity_state_value, text=hum_state, fill=hum_color
            )
            ...
            self.bg_canvas.itemconfig(
                self.aqi_state_value, text=aqi_state, fill=aqi_color
            )
    ...
    def show_coordinates(self, event):
        ...
        marker = self.bg_canvas.create_oval(x - 2, y - 2, x + 2, y + 2, fill="red")
        text = self.bg_canvas.create_text(
            x + 10, y - 10, text=f"({x}, {y})", fill="red", anchor="w"
        )
    ...
    def get_sun_info(self):
        ...
        try:
            ...
            if not os.path.exists(sun_data_file):
                self.log(
                    f"Sun data file not found: {sun_data_file}", level=logging.WARNING
                )
            ...
            self.log(
                f"No sun data found for date: {current_date}", level=logging.WARNING
            )
            return {"sunrise": "06:00", "sunset": "18:00"}
