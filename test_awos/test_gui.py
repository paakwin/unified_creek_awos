...
class WeatherStationSystem:
    ...
    def update_display(self):
        ...
        try:
            ...
            self.bg_canvas.itemconfig(
                self.temperature_value, text=f"{self.sensor_data['temperature']:.1f}"
            )
            self.bg_canvas.itemconfig(
                self.humidity_value, text=f"{self.sensor_data['humidity']:.1f}%"
            )
            self.bg_canvas.itemconfig(
                self.pressure_value, text=f"{self.sensor_data['pressure']:.1f}"
            )
            self.bg_canvas.itemconfig(
                self.wind_speed_value, text=f"{self.sensor_data['wind_speed']:.1f}"
            )
            self.bg_canvas.itemconfig(
                self.wind_direction_value, text=f"{self.sensor_data['wind_direction']}°"
            )
            self.bg_canvas.itemconfig(
                self.rain_value, text=f"{self.sensor_data['rainfall']:.1f}"
            )
            self.bg_canvas.itemconfig(
                self.uv_value, text=f"{self.sensor_data['uv_index']:.1f}"
            )
            self.bg_canvas.itemconfig(
                self.aqi_value, text=f"{self.sensor_data['aqi']:.0f}"
            )
    ...
    def update_state_displays(self):
        ...
        self.bg_canvas.itemconfig(
            self.humidity_state_value, text=humidity_state, fill=humidity_color
        )
    ...
    def update_static_elements(self):
        ...
        self.bg_canvas.itemconfig(
            self.current_day_value, text=self.custom_date.strftime("%A").upper()
        )
        self.bg_canvas.itemconfig(
            self.current_date_value, text=self.custom_date.strftime("%d %b %Y").upper()
        )
        self.bg_canvas.itemconfig(
            self.current_time_value, text=datetime.now().strftime("%H:%M")
        )  # Changed from '%H:%M:%S' to '%H:%M'
    ...
    def calculate_sun_times(self, date):
        ...
        return (
            f"{sunrise_h:02d}:{sunrise_min:02d}",
            f"{sunset_h:02d}:{sunset_min:02d}",
        )
