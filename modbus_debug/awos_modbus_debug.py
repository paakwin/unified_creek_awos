...
log_file = os.path.join(log_dir, f'debug_modbus_{datetime.now().strftime("%Y-%m-%d")}.log')
logging.basicConfig(
    level=logging.INFO,
...
class SensorDebugger:
    ...
    def init_modbus(self) -> None:
        ...
        logger.info(f"Modbus config: port={self.config['modbus']['port']}, baudrate={self.config['modbus']['baudrate']}")
    ...
    def read_wind_direction(self) -> dict:
        ...
        with self.modbus_lock:
            try:
                ...
                result = self.modbus_client.read_holding_registers(
                    address=0x0000, count=1, slave=self.config['sensors']['wind_direction'])
                if result.isError():
                    logger.error("Wind direction sensor read error: Modbus request failed")
                    return None
        ...
        while self.running:
            try:
                ...
                with self.modbus_lock:
                    ...
                    for attempt in range(self.config['modbus']['retries']):
                        if self.modbus_client.connect():
                            logger.info("Modbus connection successful in sensor_reader_loop")
                            connected = True
                            break
                        logger.warning(f"Modbus connection attempt {attempt + 1} failed")
                        time.sleep(1)
